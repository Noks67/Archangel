...
local ru_trashtalk = {
    ...
    {'иди прикупи archangel а то падаешь', 'ссылку кинуть?'},
    {'хпахпахахахаха', 'ебать я тебе запромежбулил'},
    {'твоя сука носит спид моя сука харли квин'},
    {'бай archangel уебище', 'иди прикупи'},
    {'A R C H A N G E L', 'лучшая луашка на скит'},
    {'1.', 'ебаный уебок'},
    {'1', 'шлюха'},
    {'t1', 'пидорас ебаный'},
    {'е1'},
...
client.color_log(200, 180, 100, ' ')
client.color_log(200, 180, 100, 'Welcome to ARCHANGEL, ', name, '! Your branch is '..build)
...
local lua_ui = {
    main = {
        main_checkbox = groups.group_fl:checkbox('\v•\a'..get_skeet_color()..' Enable ARCHANGEL'),
    tab_selectable = groups.group_fl:combobox('\v•\r Select Tab', { 'Ragebot', 'Anti-Aims', 'Visuals', 'Misc' }),
    label1 = groups.group_fl:label('\v•\r Welcome to \a'..get_skeet_color()..'ARCHANGEL\r '..build..', '..name),
    ...
    auto_tp_key       = groups.group_aa:hotkey('\v•\r Teleport Hotkey', true),

    -- Отступ
    ragebot_label9    = groups.group_aa:label(' '),

    -- Resolver
    resolver          = groups.group_aa:checkbox('\v•\r ARCH Resolver [debug]'),
    resolver_type     = groups.group_aa:combobox('\v•\r Resolver Type', {'Default', 'Experimental'}),

    -- Разделитель
    ragebot_label4    = groups.group_aa:label('》═══════~◈~═══════《'),

    -- Prediction System
    prediction_system = groups.group_aa:checkbox('\v•\r ARCH Prediction System [debug]'),
    predict_label     = groups.group_aa:label('\aEC3F3FFFUse if ping < 40'),
    predict_type      = groups.group_aa:combobox('\v•\r Prediction Mode', {'Default', 'Aggressive'}),
    predict_label2    = groups.group_aa:label('\aEC3F3FFFUse only if ping < 20'),
    ...
    predict_bind      = groups.group_aa:hotkey('\v•\r Prediction Override Key'),
    ...
    fake_lags_amount        = groups.group_aa:combobox('\v•\r  Type', {'Maximum', 'Dynamic', 'Fluctuate', 'ARCHANGEL'}),
    ...
    other = {
        ...
        hitmarker_type = groups.group_aa:combobox('\v•\r  Type', {'Heart', 'Star'}),
        ...
        log_type = groups.group_aa:multiselect('\v•\r  Log Types', {'Hit', 'Miss'}),
        ...
        keybinds_enable = groups.group_aa:checkbox('\v•\r Keybinds List', {160, 200, 255}),
keybinds_label = groups.group_aa:label(' ')
    },
...
lua_ui.other.keybinds_enable:depend(enabled, visuals_tab)
lua_ui.other.keybinds_label:depend(enabled, visuals_tab)
...
local function aa_setup(cmd)
    ...
    if lua_ui.antiaim_misc.fake_lags:get() then
        if lua_ui.ragebot.hideshot_fix:get() and is_osaa and not is_dt then
            ...
            if lua_ui.antiaim_misc.fake_lags_amount:get() == 'ARCHANGEL' then
...
telegram = groups.group_fl:button('Telegram', function()
    panorama.open().SteamOverlayAPI.OpenExternalBrowserURL("https://t.me/kyberhub")
end):depend(enabled)
...
buttom_sex = groups.group_fl:button('\a'..get_skeet_color()..'ARCH Preset [\aEC3F3FFFdebug\r]', function()
...
client.set_event_callback("aim_hit", function(e)
    local target = e.target
    if target then
        hit_count[target] = (hit_count[target] or 0) + 1
        miss_count[target] = 0
    end
end)
...
expres.handle = function(current_threat)
    ...
    if expres.body_yaw[current_threat] == nil then
        ...
        hit_count[current_threat], miss_count[current_threat] = 0, 0
        last_side[current_threat] = 1
    end
    ...
    if expres.body_yaw[current_threat][prev_simtime] ~= nil then
        ...
        local curr_side = (globals.tickcount() % 2 == 0 and 1 or -1)

        -- RESOLVER LOGIC
        if lua_ui.ragebot.resolver:get() then
            local resolver_type = lua_ui.ragebot.resolver_type:get()
            ...
            if resolver_type == "???" then
                ...
                -- Smart switching after N misses
                if miss_count[current_threat] and miss_count[current_threat] >= 2 then
                    value_body = -value_body
                    last_side[current_threat] = -last_side[current_threat]
                end
            ...
            elseif resolver_type == "Default" then
                local side = last_side[current_threat] or 1
                local value_body = 0

                if pitch_e then
                    value_body = 0
                else
                    value_body = side * (max_desync * math.random(24, 58))
                end
...
local function crosshair_indicators()
    ...
    local ind_size = renderer.measure_text('cb', 'archangel')
    ...
    if lua_ui.other.cross_ind_gradient:get() then
        text_fade_animation(center[1] + scoped_space, center[2] + 25, -1, {r=r1, g=g1, b=b1, a=255}, {r=r2, g=g2, b=b2, a=255}, new_check and string.upper('ARCHANGEL') or 'ARCHANGEL', main_font)
    else
        renderer.text(center[1] + scoped_space, center[2] + 25, r2, g2, b2, 255, main_font, 0, 'ARCHANGEL')
    end
...
local function manual_indicator()
    ...
    if yaw_direction == 90 then
        if not lua_ui.other.manual_indicators_velocity:get() then
            renderer.text(center[1] + 60, center[2] - scoped_space_manual, r2, g2, b2, 255, 'c+', 0, right_arrow)
        else
            renderer.text(center[1] + 60 + man_velocity/7, center[2] - scoped_space_manual, r2, g2, b2, 255, 'c+', 0, right_arrow)
        end
    else
        if not lua_ui.other.manual_indicators_velocity:get() then
            renderer.text(center[1] + 60, center[2] - scoped_space_manual, 100, 100, 100, 100, 'c+', 0, right_arrow)
        else
            renderer.text(center[1] + 60 + man_velocity/7, center[2] - scoped_space_manual, 100, 100, 100, 100, 'c+', 0, right_arrow)
        end
    end
    -- Левая стрелка ⮐
    local left_arrow = '⮐'
    if yaw_direction == -90 then
        if not lua_ui.other.manual_indicators_velocity:get() then
            renderer.text(center[1] - 60, center[2] - scoped_space_manual, r2, g2, b2, 255, 'c+', 0, left_arrow)
        else
            renderer.text(center[1] - 60 - man_velocity/7, center[2] - scoped_space_manual, r2, g2, b2, 255, 'c+', 0, left_arrow)
        end
    else
        if not lua_ui.other.manual_indicators_velocity:get() then
            renderer.text(center[1] - 60, center[2] - scoped_space_manual, 100, 100, 100, 100, 'c+', 0, left_arrow)
        else
            renderer.text(center[1] - 60 - man_velocity/7, center[2] - scoped_space_manual, 100, 100, 100, 100, 'c+', 0, left_arrow)
                end
            end
...
local function watermark()
    ...
    local build_str = string.upper(build or "BETA")
    local user = entity.get_player_name(me) or "PLAYER"

    local text = "ARCHANGEL [" .. build_str .. "] | " .. user .. " | " .. latency .. "MS"
    local font_flags = "b" -- matches style 'small' (skeet-thin)

    -- измеряем текст
    local text_w, text_h = renderer.measure_text(font_flags, text)
    local x = math.floor((screen_w - text_w) / 2)
    local y = screen_h - text_h - 26

    -- фон с мягким градиентом
    renderer.gradient(x, y + text_h + 6, text_w / 2, 1, 12, 12, 12, 0, 220, 220, 220, 220, true)
    renderer.gradient(x + text_w / 2, y + text_h + 6, text_w / 2, 1, 220, 220, 220, 220, 12, 12, 12, 0, true)
    renderer.gradient(x, y, text_w / 2, text_h + 5, 12,12,12,0, 12,12,12,75, true)
    renderer.gradient(x + text_w / 2, y, text_w / 2, text_h + 5, 12,12,12,75, 12,12,12,0, true)

    -- основной текст
    renderer.text(x + 1, y + 1, 0, 0, 0, 180, font_flags, 0, text)  -- тень
    renderer.text(x, y, 255, 255, 255, 255, font_flags, 0, text)    -- текст
end
...
local function velocity_ind()
    ...
    local danger_velocity_threshold = 88  -- ниже — опасная зона

    -- Цвета из UI или красный при опасной скорости
    local r, g, b, a = lua_ui.other.velocity_warning:get_color()
    if velocity < danger_velocity_threshold then
        r, g, b = 255, 50, 50  -- красный при низкой скорости
    end

    local alpha = math.min(math.floor(math.sin((globals.curtime() % 3) * 10) * 175 + 50), 255)

    local base_y = screen[2] / 3
    local bar_max = 200
    local bar_w = math.min(bar_max, math.max(0, (velocity * 2)))

    local text_icon = velocity < danger_velocity_threshold and "⚠ " or "🚀 "
    local text_label = "* VELOCITY *"
    local full_text = text_icon .. text_label

    if velocity < 100 or ui.is_menu_open() then
        -- Текст с иконкой
        renderer.text(center[1], base_y - 18, 255, 255, 255, alpha, 'cb', 0, full_text)

        -- Фон под полосой
        renderer.rounded_rectangle(center[1] - bar_max / 2 - 2, base_y, bar_max + 4, 6, 12, 12, 12, 220, 3)

        -- Прогресс-бар
        renderer.rounded_rectangle(center[1] - bar_max / 2, base_y + 2, bar_w - 1, 2, r, g, b, 255, 2)
    end
end

client.set_event_callback("paint", velocity_ind)
...
local function defensive_ind()
    ...
    local tickbase_inds = entity.get_prop(lp, "m_nTickBase") or 0
    ...
    local bar_max = 100
    local bar_w = defensive_ticks_left_inds * (bar_max / 14)

    -- Alpha logic
    if not ui.is_menu_open() then
        if (ui.get(ref.dt[1]) and ui.get(ref.dt[2])) or
           (ui.get(ref.os[1]) and ui.get(ref.os[2])) or
           not ui.get(ref.fakeduck) then
            if exploit_charged() and active then
                defensive_alpha = math.lerp(defensive_alpha, 255, 0.3)
            elseif exploit_charged() and not active then
                defensive_alpha = math.lerp(defensive_alpha, 150, 0.2)
            else
                defensive_alpha = math.lerp(defensive_alpha, 0, 0.2)
            end
        else
            defensive_alpha = math.lerp(defensive_alpha, 0, 0.2)
        end
    ...
    -- Draw
    if defensive_alpha > 5 then
        local base_y = screen[2] / 3.5

        -- Label
        renderer.text(center[1], base_y - 18, r, g, b, math.floor(defensive_alpha), "cb", 0, "★ DEFENSIVE MODE")

        -- Background bar
        renderer.rounded_rectangle(center[1] - bar_max / 2 - 2, base_y, bar_max + 4, 6, 12, 12, 12, math.floor(defensive_alpha * 0.5), 3)

        -- Progress bar
        if bar_w > 0 then
            renderer.rounded_rectangle(center[1] - bar_max / 2, base_y + 2, bar_w, 2, r, g, b, math.floor(defensive_alpha), 2)
        end
    end
end

client.set_event_callback("paint", defensive_ind)
    ...
    for tick, data in pairs(queue) do
        if globals.curtime() <= data[4] then
            ...
            if x1_hit ~= nil and y_hit ~= nil then
                local type = lua_ui.other.hitmarker_type:get()
                if type == 'Heart' then
                    renderer.text(x1_hit, y_hit, r1, g1, b1, 255, "c+", 0, "❤")
...
client.set_event_callback('aim_hit', function(e)
    if lua_ui.main.main_checkbox:get() and lua_ui.other.aimbot_logs:get() and lua_ui.other.log_type:get('Hit') then
	    ...
	    client.color_log(124, 252, 100, "ARCHANGEL ~ \0")
...
client.set_event_callback("aim_miss", function(e)
    if lua_ui.main.main_checkbox:get() and lua_ui.other.aimbot_logs:get() and lua_ui.other.log_type:get('Miss') then
	    local group = hitgroup_names[e.hitgroup + 1] or "?"
	    client.color_log(255, 100, 100, "ARCHANGEL ~ \0")
...
--region keybinds system
local keybinds_system = {}

-- Позиция keybinds (фиксированная)
keybinds_system.x = 700
keybinds_system.y = 400
...
local function measure_text(font, text)
    return renderer.measure_text(font, text)
end

-- Функция подсчета активных элементов
local function item_count(table)
    local count = 0
    for _ in pairs(table) do count = count + 1 end
    return count
end

-- Главные ссылки на hotkey'и из volta
local keybind_refs = {
    -- Manual directions
    {'Manual Left', function() return lua_ui.antiaim_misc.key_left:get() end},
    {'Manual Right', function() return lua_ui.antiaim_misc.key_right:get() end},
    {'Manual Forward', function() return lua_ui.antiaim_misc.key_forward:get() end},
    {'Freestanding', function() return lua_ui.antiaim_misc.key_freestand:get() end},

    -- Ragebot features
    {'Auto Teleport', function() return lua_ui.ragebot.auto_tp and lua_ui.ragebot.auto_tp_key:get() end},
    {'Air Quick Stop', function() return lua_ui.ragebot.airstop and lua_ui.ragebot.airstop:get() end},

    -- Defensive features
    {'Defensive Flick', function() return lua_ui.antiaim_misc.defensive_flick_key and lua_ui.antiaim_misc.defensive_flick_key:get() end},

    -- Gamesense built-in references
    {'Double Tap', function() return ui.get(ref.dt[1]) and ui.get(ref.dt[2]) end},
    {'On Shot AA', function() return ui.get(ref.os[1]) and ui.get(ref.os[2]) end},
    {'Force Safe Point', function() return ui.get(ref.safepoint) end},
    {'Force Body Aim', function() return ui.get(ref.forcebaim) end},
    {'Slow Motion', function() return ui.get(ref.slow[1]) and ui.get(ref.slow[2]) end},
    {'Auto Peek', function() return ui.get(ref.autopeek[1]) and ui.get(ref.autopeek[2]) end},
    {'Minimum Damage', function() return ui.get(ref.minimum_damage_override[1]) and ui.get(ref.minimum_damage_override[2]) end},
    {'Ping Spike', function() return ui.get(ref.ping_spike[1]) and ui.get(ref.ping_spike[2]) end},
}

-- Основная функция отрисовки keybinds
local function paint_keybinds()
    if not lua_ui.main.main_checkbox:get() or not lua_ui.other.keybinds_enable:get() then
        GLOBAL_ALPHA = math.lerp(GLOBAL_ALPHA, 0, 15)
        if GLOBAL_ALPHA < 0.01 then return end
    end

    local menu_open = ui.is_menu_open()
    local frames = globals.absoluteframetime() * 8
    local latest_item = false
    local maximum_offset = 0

    -- Обновляем состояние всех кейбиндов
    for _, bind_data in pairs(keybind_refs) do
        local bind_name = bind_data[1]
        local bind_func = bind_data[2]

        local active = false
        if bind_func then
            local success, result = pcall(bind_func)
            active = success and result
        end

        if active then
            latest_item = true
            if active_keybinds[bind_name] == nil then
                active_keybinds[bind_name] = {
                    mode = 'Toggle',
                    alpha = 0,
                    offset = 0,
                    active = true
            ...
            local text_width = measure_text(nil, bind_name)
            active_keybinds[bind_name].active = true
            active_keybinds[bind_name].offset = text_width
            active_keybinds[bind_name].alpha = math.min(active_keybinds[bind_name].alpha + frames, 1)
        elseif active_keybinds[bind_name] ~= nil then
            active_keybinds[bind_name].active = false
            active_keybinds[bind_name].alpha = active_keybinds[bind_name].alpha - frames
            if active_keybinds[bind_name].alpha <= 0 then
                active_keybinds[bind_name] = nil
            end
        end

        if active_keybinds[bind_name] ~= nil and active_keybinds[bind_name].offset > maximum_offset then
            maximum_offset = active_keybinds[bind_name].offset
        end
    end

    -- Добавляем "Menu toggled" если меню открыто
    if menu_open then
    ...
    -- Рисуем keybinds список
    local title = 'keybinds'
    local x, y = keybinds_system.x, keybinds_system.y
    local r, g, b, a = lua_ui.other.keybinds_enable:get_color()
    local height_offset = 23
    local w, h = 75 + maximum_offset, 50

    -- Верхняя полоска
    renderer.rectangle(x, y, w, 2, r, g, b, GLOBAL_ALPHA * 190)  -- чуть ярче и прозрачнее

    -- Фон
    renderer.rectangle(x, y + 2, w, 22, 25, 25, 25, GLOBAL_ALPHA * a * 0.85)

    -- Заголовок
    renderer.text(x + w/2, y + 8, 245, 245, 245, GLOBAL_ALPHA * 255, 'c', 0, title)

    -- Фон для блока с кейбиндами (светло-тёмно-серый с небольшой прозрачностью)
renderer.rectangle(x, y + 24, w, h - 24, 15, 15, 15, GLOBAL_ALPHA * a * 0.75)

-- Легкая рамка вокруг всего блока (тень или контур)
renderer.rectangle(x - 1, y - 1, w + 2, h + 2, 40, 40, 40, GLOBAL_ALPHA * 100)

    -- Рисуем активные кейбинды
    local height_offset = 28
for bind_name, bind_data in pairs(active_keybinds) do
    local key_type = '[' .. bind_data.mode .. ']'
    renderer.text(x + 10, y + height_offset, 220, 220, 220, GLOBAL_ALPHA * bind_data.alpha * 255, '', 0, bind_name)
    renderer.text(x + w - measure_text(nil, key_type) - 10, y + height_offset, 200, 200, 200, GLOBAL_ALPHA * bind_data.alpha * 255, '', 0, key_type)
    height_offset = height_offset + 18
end

    -- Обновляем общую альфу
    if lua_ui.main.main_checkbox:get() and lua_ui.other.keybinds_enable:get() and item_count(active_keybinds) > 0 and latest_item then
        GLOBAL_ALPHA = math.min(GLOBAL_ALPHA + frames, 1)
    else
        GLOBAL_ALPHA = math.max(GLOBAL_ALPHA - frames, 0)
    end

    -- Убираем "Menu toggled" если меню закрыто
    if not menu_open then
        active_keybinds['Menu toggled'] = nil
    end
end

--endregion keybinds system
